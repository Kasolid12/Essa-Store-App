...
10. ✅ Session 11 (2026-07-10): Optimasi Pengeluaran Offline — Keep nama person setelah simpan (multiple input); Tambah SUPPLIER ke dropdown pembeli untuk piutang otomatis.
...
| Task | Status |
|------|--------|
| Optimasi Catatan Harian — Pengeluaran Offline | ✅ Done |

Feature:
1. **Keep nama person** — `submit_offline()` sekarang simpan nama Pembeli/Supplier sebelum reset form, lalu restore setelahnya. Cocok untuk input multiple penjualan ke orang yang sama.
2. **SUPPLIER dropdown** — `load_persons()` sekarang include `'SUPPLIER'` di dropdown person_off, jadi supplier (Kak Yudi, Kak Hari, dll) bisa dicatat penjualannya dan otomatis masuk piutang via `_sync_client_receivable()`.

File terkunci: `ui/views/harian_view.py`.
